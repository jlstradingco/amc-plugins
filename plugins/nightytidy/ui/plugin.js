...
function renderTabBar() {
  ...
  document.getElementById('tab-bar').innerHTML = tabs
    .map(
      (t) =>
        `<button class="tab-btn ${t.id === currentView ? 'active' : ''}" data-view="${t.id}" onclick="showView('${t.id}')">${t.label}</button>`
    )
    .join('')
}
...
function renderProjectSelector() {
  if (state.projects.length === 0) return '<p class="text-muted">No projects found</p>'
  return `
      ...
      <select id="project-select" onchange="onProjectChange(this.value)">
        ...
        ${state.projects
          ...
          <option value="${p.id}" ${p.id === state.selectedProjectId ? 'selected' : ''}>
            ${escapeHtml(p.name)} \u2014 ${escapeHtml(p.folderPath)}
          </option>
        `
          )
          .join('')}
...
function renderDashboardStats(runs) {
  ...
  return `
    <div class="stats-row">
      <div class="stat-card">
        <div class="stat-value">${new Date(lastRun.started_at).toLocaleDateString()}</div>
...
async function loadDashboard() {
  ...
  if (!state.selectedProjectId) {
    container.innerHTML = `
      ...
      <div class="empty-state">
        ...
        <p>Select a project to get started with automated overnight improvements.</p>
      </div>
    `
    return
  }
  ...
  try {
    state.runs = await amc.nightytidy.getRunHistory(state.selectedProjectId)
  } catch (err) {
    state.runs = []
  }
  ...
  container.innerHTML = `
    ...
    <div class="toolbar">
      <div></div>
      <button class="btn-primary" onclick="showView('step-selection'); loadStepSelection()">
        Start Run
      </button>
    </div>
    ${
      state.runs.length > 0
        ...
        : `<div class="empty-state">
          <h3>No runs yet</h3>
          <p>Start your first NightyTidy run to automatically improve this codebase.</p>
          <button class="btn-primary" onclick="showView('step-selection'); loadStepSelection()">Run First Improvement</button>
        </div>`
    }
  `
}
...
async function loadStepSelection() {
  const container = document.getElementById('view-step-selection')

  if (!state.selectedProjectId) {
    container.innerHTML =
      '<div class="empty-state"><h3>Select a project first</h3><p>Go to Dashboard and choose a target project.</p></div>'
    return
  }
  ...
  try {
    ...
    container.innerHTML = `<div class="card" style="color:var(--error)">Failed to load steps: ${escapeHtml(err.message)}</div>`
    return
  }
  ...
  renderStepSelection(container)
}

function renderStepSelection(container) {
  ...
  container.innerHTML = `
    <div class="toolbar">
      <div style="display:flex;gap:8px;align-items:center">
        <button class="btn-secondary" onclick="selectAllSteps()">Select All</button>
        <button class="btn-secondary" onclick="selectNoSteps()">Select None</button>
        <select id="preset-select" onchange="loadPreset(this.value)" style="width:200px">
          <option value="">Load preset...</option>
          <option value="__all__">Full Audit (all ${enabledSteps.length})</option>
          <option value="__quick__">Quick Sweep (10 steps)</option>
          ${state.presets.map((p) => `<option value="${p.id}">${escapeHtml(p.name)}</option>`).join('')}
        </select>
        <button class="btn-secondary" onclick="saveCurrentAsPreset()">Save as Preset</button>
      </div>
      <div style="display:flex;gap:8px;align-items:center">
        <label class="form-label" style="margin:0">Timeout:</label>
        <input type="number" id="step-timeout" value="${state.settings.defaultTimeout || 45}" min="5" max="120" style="width:80px"> min
      </div>
    </div>
      ...
      .map(([category, steps]) => {
        ...
        <div class="category-group">
          <div class="category-header" onclick="toggleCategoryCollapse('${category}')">
            ...
            <span class="category-count">${catSelected}/${steps.length} selected</span>
            <button class="btn-secondary" style="padding:2px 8px;font-size:11px;margin-left:auto" onclick="event.stopPropagation(); selectCategory('${category}')">Select All</button>
          </div>
    ...
    <div style="padding:16px 0;display:flex;justify-content:flex-end;gap:12px;border-top:1px solid var(--border);margin-top:16px">
      <button class="btn-secondary" onclick="showView('dashboard'); loadDashboard()">Cancel</button>
      <button class="btn-primary" ${selectedCount === 0 ? 'disabled' : ''} onclick="startNewRun()">
        Start Run (${selectedCount} step${selectedCount !== 1 ? 's' : ''})
      </button>
    </div>
  `
}
...
function toggleStep(stepId) {
  if (state.selectedStepIds.has(stepId)) {
    state.selectedStepIds.delete(stepId)
  } else {
    state.selectedStepIds.add(stepId)
  }
  renderStepSelection(document.getElementById('view-step-selection'))
}
...
function selectCategory(category) {
  state.steps
    ...
    .forEach((s) => state.selectedStepIds.add(s.id))
  renderStepSelection(document.getElementById('view-step-selection'))
}
...
function loadPreset(presetId) {
  ...
  if (preset) {
    ...
    state.steps
      .filter((s) => stepIds.includes(s.step_id))
      .forEach((s) => state.selectedStepIds.add(s.id))
    renderStepSelection(document.getElementById('view-step-selection'))
  }
}
...
async function saveCurrentAsPreset() {
  ...
  try {
    await amc.nightytidy.savePreset({
      name,
...
async function loadLiveRun() {
  if (!state.activeRun) {
    document.getElementById('view-live-run').innerHTML =
      '<div class="empty-state"><h3>No active run</h3></div>'
    return
  }
...
async function refreshLiveRun() {
  ...
  try {
    ...
    if (['completed', 'failed', 'partial', 'cancelled'].includes(status.run.status)) {
      if (liveRunPollInterval) {
        clearInterval(liveRunPollInterval)
        liveRunPollInterval = null
      }
    }
  } catch (err) {
    console.error('[NightyTidy] Poll error:', err)
  }
}
...
function renderLiveRun() {
  ...
  if (!run) {
    container.innerHTML = '<div class="empty-state"><h3>No active run</h3></div>'
    return
  }
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
        ...
        <div style="display:flex;gap:8px">
          ${
            isRunning
              ? `
            <button class="btn-secondary" onclick="skipCurrentStep()">Skip Step</button>
            <button class="btn-danger" onclick="cancelActiveRun()">Cancel Run</button>
          `
              : `
    ...
    <div style="margin-top:16px">
      ${steps
        .map((step) => {
            ...
            step.status === 'completed'
              ...
              : step.status === 'failed'
                ...
                : step.status === 'running'
                  ...
                  : 'var(--text-muted)'
          return `
          <div class="step-row" style="border-left:3px solid ${color};margin-bottom:4px">
          ...
          ${
            step.summary && (step.status === 'completed' || step.status === 'failed')
              ? `
            <div style="padding:4px 12px 8px 35px;font-size:12px;color:var(--text-secondary)">${escapeHtml(step.summary)}</div>
          `
              : ''
          }
        `
...
async function skipCurrentStep() {
  amc.toast.show({
    type: 'info',
    message: 'Step will complete or timeout based on the configured timeout setting.'
  })
}
...
async function viewRunResults(runId) {
  ...
  container.innerHTML =
    '<div style="text-align:center;padding:40px"><span class="spinner"></span> Loading results...</div>'

  try {
    const data = await amc.nightytidy.getRunStatus(runId)
    if (!data) {
      container.innerHTML = '<div class="empty-state"><h3>Run not found</h3></div>'
      return
    }
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
        <div>
          ...
          <div style="font-size:12px;color:var(--text-muted)">
            ...
            ${duration ? ` &mdash; ${formatDurationShort(duration)}` : ''}
          </div>
        </div>
        ...
        const color =
          step.status === 'completed'
            ...
            : step.status === 'failed'
              ...
              : 'var(--text-muted)'
        return `
        <div class="card" style="border-left:3px solid ${color};margin-bottom:8px">
          ...
          <div class="accordion-body" id="result-${i}">
            ...
            ${
              step.output
                ? `
              <details>
                <summary style="cursor:pointer;font-size:12px;color:var(--text-muted)">Full Output</summary>
                <pre style="background:var(--bg-secondary);padding:12px;border-radius:6px;font-size:12px;overflow-x:auto;margin-top:8px;white-space:pre-wrap">${escapeHtml(step.output)}</pre>
              </details>
            `
                : ''
            }
...
async function rollbackRunConfirm(runId) {
  if (!confirm('This will delete the NightyTidy branch and revert all changes. Are you sure?'))
    return
  try {
...
async function rerunFailed(runId) {
  ...
  state.steps
    .filter((s) => failedStepIds.includes(s.step_id))
    .forEach((s) => state.selectedStepIds.add(s.id))
  showView('step-selection')
  loadStepSelection()
}
...
async function loadSettings() {
  ...
  try {
    ...
    state.steps = await amc.nightytidy.getStepLibrary()
    state.schedules = state.selectedProjectId
      ? await amc.nightytidy.getSchedules(state.selectedProjectId)
      : []
  } catch (err) {
    container.innerHTML = `<div class="card" style="color:var(--error)">${escapeHtml(err.message)}</div>`
    return
...
function renderSettingsView(container) {
  container.innerHTML = `
    ...
    <div class="card">
      <div class="form-group">
        <label class="form-label">Step Timeout (minutes)</label>
        <input type="number" id="setting-timeout" value="${state.settings.defaultTimeout ?? 45}" min="5" max="120">
      </div>
      <div class="form-group">
        <label class="form-label">Max Retries Per Step</label>
        <input type="number" id="setting-retries" value="${state.settings.maxRetries ?? 3}" min="1" max="5">
      </div>
      <div class="form-group">
        <label class="form-label">Fast Completion Threshold (minutes)</label>
        <input type="number" id="setting-fast" value="${state.settings.fastCompletionThreshold ?? 2}" min="1" max="10">
      </div>
      <div class="form-group" style="display:flex;justify-content:space-between;align-items:center">
        <label class="form-label" style="margin:0">Generate Report</label>
        <input type="checkbox" id="setting-report" ${state.settings.generateReport !== false ? 'checked' : ''}>
      </div>
      <div class="form-group" style="display:flex;justify-content:space-between;align-items:center">
        <label class="form-label" style="margin:0">Auto-Merge on Success</label>
        <input type="checkbox" id="setting-automerge" ${state.settings.autoMerge ? 'checked' : ''}>
      </div>
    ...
    <div style="margin-bottom:12px">
      <button class="btn-secondary" onclick="showAddStepForm()">+ Add Custom Step</button>
    </div>
    <div id="step-library-list">
      ${state.steps
        .map(
          (step) => `
        <div class="step-row" style="margin-bottom:4px;border-left:3px solid ${CATEGORY_COLORS[step.category] || '#64748b'}">
          <span style="color:${CATEGORY_COLORS[step.category] || '#64748b'}">&#9679;</span>
          ...
          <span class="step-category-tag" style="background:${CATEGORY_COLORS[step.category] || '#64748b'}20;color:${CATEGORY_COLORS[step.category] || '#64748b'}">${CATEGORY_LABELS[step.category] || step.category}</span>
          <label style="font-size:12px;display:flex;align-items:center;gap:4px">
            <input type="checkbox" ${step.enabled ? 'checked' : ''} onchange="toggleStepEnabled('${step.id}', this.checked)"> Enabled
          </label>
...
function renderSchedules() {
  if (state.schedules.length === 0)
    return '<p style="color:var(--text-muted);margin-bottom:12px">No schedules configured.</p>'
      ...
      <div style="display:flex;justify-content:space-between;align-items:center">
        <div>
          <div class="day-toggles">
            ${DAY_NAMES.map((d, i) => `<span class="day-toggle ${(sch.days || []).includes(i) ? 'active' : ''}" style="pointer-events:none">${d}</span>`).join('')}
          </div>
          <div style="font-size:12px;color:var(--text-muted);margin-top:4px">at ${sch.time || '23:00'}</div>
        </div>
        <div style="display:flex;gap:8px;align-items:center">
          <label style="font-size:12px;display:flex;align-items:center;gap:4px">
            <input type="checkbox" ${sch.enabled ? 'checked' : ''} onchange="toggleSchedule('${sch.id}', this.checked)"> Active
          </label>
...
async function saveSettings() {
  amc.toast.show({
    type: 'info',
    message: 'Settings are managed via AMC Settings > Plugins > NightyTidy'
  })
}

async function toggleStepEnabled(stepId, enabled) {
  try {
    await amc.nightytidy.updateStep(stepId, { enabled: enabled ? 1 : 0 })
    state.steps = await amc.nightytidy.getStepLibrary()
  } catch (err) {
    amc.toast.show({ type: 'error', message: err.message })
  }
...
function showAddStepForm() {
  ...
  const category = prompt(
    'Category (documentation, testing, security, performance, architecture, ux, infrastructure, product):'
  )
  if (!category) return
...
async function createNewSchedule(days, time) {
  try {
    await amc.nightytidy.createSchedule({
      project_id: state.selectedProjectId,
      days,
      time,
      step_preset_id: null,
...
async function init() {
  ...
  amc.theme.onChange((t) => {
    document.documentElement.setAttribute('data-theme', t.mode === 'light' ? 'light' : '')
  })
